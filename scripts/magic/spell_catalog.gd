...
static func build_cast_command(spell_id: String, caster_state: Dictionary, now_ms: int, sequence: int, target: Dictionary = {}) -> Dictionary:
	...
	return {"accepted": true, "reason": "", "command": {"format_version": FORMAT_VERSION, "command_id": command_id, "caster_id": caster_id, "spell_id": spell_id, "school": str(spell.get("school", "")), "resource": str(spell.get("resource", "")), "resource_cost": int(spell.get("cost", 0)), "issued_at_ms": now_ms, "ready_at_ms": now_ms + cooldown_ms, "cast_time_ms": roundi(float(spell.get("cast_time", 0.0)) * 1000.0), "targeting": str(spell.get("targeting", "")), "target": target.duplicate(true), "range": float(spell.get("range", 0.0)), "power": float(spell.get("power", 0.0)), "tags": (spell.get("tags", []) as Array).duplicate()}}
...
static func apply_cast_cost(command: Dictionary, caster_state: Dictionary) -> Dictionary:
	...
	resources[resource_name] = maxi(0, int(resources.get(resource_name, 0)) - int(command.get("resource_cost", 0)))
