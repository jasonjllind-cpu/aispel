...
func _run() -> void:
	...
	combat.name = "NetworkCombatAuthority"
	...
	if hit.get("ok", false) != true or int(hit.get("damage", 0)) != 16 or int(enemy.get("health")) != 34:
		_fail(root, world_state, "Valid server-authoritative attack did not apply expected damage")
		return
	...
	if remote_damage.get("ok", false) != true or int((players.states[4] as Dictionary).get("health", -1)) != 67:
		_fail(root, world_state, "Server did not own remote player damage")
		return
...
func _fail(root: Node, world_state: Node, message: String) -> void:
