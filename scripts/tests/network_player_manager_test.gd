...
func _run() -> void:
	...
	# Keep manager outside the tree to avoid production deferred installation
	# racing this explicit validation contract.
	...
	if str(spoof_result.get("error", "")) != "player_spoof":
		_fail(manager, root, "Player identity spoof was not rejected")
		return
	...
	if str(bounds_result.get("error", "")) != "position_out_of_bounds":
		_fail(manager, root, "Out-of-bounds player state was not rejected")
		return
	...
	if str(teleport_result.get("error", "")) != "movement_step_too_large":
		_fail(manager, root, "Implausible movement snapshot was not rejected")
		return
	...
	if plausible_result.get("ok", false) != true:
		_fail(manager, root, "Plausible movement snapshot was rejected")
		return
	...
	if str(unknown_result.get("error", "")) != "unknown_peer":
		_fail(manager, root, "Unknown peer player state was not rejected")
		return
	manager.call("remove_peer", 4)
	if not (manager.call("state_for_peer", 4) as Dictionary).is_empty():
		_fail(manager, root, "Disconnected peer state was not removed")
		return
