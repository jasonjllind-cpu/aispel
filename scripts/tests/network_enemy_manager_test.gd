...
func _run() -> void:
	...
	manager.name = "NetworkEnemyManager"
	...
	if states.size() != 1 or str((states[0] as Dictionary).get("id", "")) != "enemy:replication:test":
		_fail(root, "Server enemy batch did not contain the stable enemy")
		return
	...
	if applied.get("ok", false) != true or int(applied.get("applied", 0)) != 1:
		_fail(root, "Valid enemy snapshot batch was rejected")
		return
	if enemy.global_position != Vector3(3, 1, -4) or int(enemy.get("health")) != 60:
		_fail(root, "Enemy transform/health did not restore from authoritative snapshot")
		return
	var stale: Dictionary = manager.call("apply_enemy_batch", batch)
	if str(stale.get("error", "")) != "stale_sequence":
		_fail(root, "Stale enemy batch was not rejected")
		return
	...
	if str(invalid.get("error", "")) != "invalid_enemy_state":
		_fail(root, "Malformed enemy snapshot was accepted")
		return
