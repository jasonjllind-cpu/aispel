...
const BOSSES: Dictionary = {
	"hollow_king": {"role": "guardian", "tier": 1, "requires_campaign": ["campaign:convergence"], "requires_world": [], "victory_flag": "boss_milestone:hollow_king_defeated"},
	"blackroot_matriarch": {"role": "guardian", "tier": 1, "requires_campaign": ["campaign:convergence"], "requires_world": [], "victory_flag": "boss_milestone:blackroot_matriarch_defeated"},
	"stormbound_titan": {"role": "guardian", "tier": 2, "requires_campaign": ["campaign:convergence"], "requires_world": ["dungeon_milestone:one_guardian_defeated"], "victory_flag": "boss_milestone:stormbound_titan_defeated"},
	"pale_abbess": {"role": "guardian", "tier": 2, "requires_campaign": ["campaign:convergence"], "requires_world": ["dungeon_milestone:one_guardian_defeated"], "victory_flag": "boss_milestone:pale_abbess_defeated"},
	"frostbound_wyrm": {"role": "guardian", "tier": 3, "requires_campaign": ["campaign:convergence"], "requires_world": ["dungeon_milestone:two_guardians_defeated"], "victory_flag": "boss_milestone:frostbound_wyrm_defeated"},
	"ashen_saint": {"role": "guardian", "tier": 3, "requires_campaign": ["campaign:convergence"], "requires_world": ["dungeon_milestone:two_guardians_defeated"], "victory_flag": "boss_milestone:ashen_saint_defeated"},
	"eclipsed_regent": {"role": "final", "tier": 4, "requires_campaign": ["campaign:endgame_unlocked"], "requires_world": ["dungeon_milestone:three_guardians_defeated"], "victory_flag": "boss_milestone:final_victory"}
}
...
static func record_authoritative_victory(state_value: Variant, boss_id: String, authority_peer_id: int, sender_peer_id: int, sequence: int) -> Dictionary:
	...
	if str(boss.get("role", "")) == "final" and int(state.get("guardian_count", 0)) < 3:
		return _result(false, "guardian_gate", state, {})
...
static func _recalculate(state: Dictionary) -> void:
	...
	for boss_id_value in state.get("defeated_boss_ids", []) as Array:
...
static func _access_decision(boss_id: String, allowed: bool, reason: String, missing: Array[String]) -> Dictionary:
	return {"format_version": FORMAT_VERSION, "decision_id": "boss_access:%s" % boss_id, "boss_id": boss_id, "allowed": allowed, "reason": reason, "missing_requirements": missing.duplicate()}

static func _result(ok: bool, error: String, state: Dictionary, event: Dictionary) -> Dictionary:
	return {"ok": ok, "error": error, "state": state.duplicate(true), "event": event.duplicate(true)}
